...
import { useSalesRegister, useExportReport } from "@/hooks/use-reports";
...
export default function SalesRegisterPage() {
  ...
  const { data, isLoading } = useSalesRegister({ fromDate, toDate, groupBy });
  ...
  const handleExport = async (format: "pdf" | "excel" | "csv") => {
    ...
    a.href = url;
    a.download = `sales-register-${today}.${format === "excel" ? "xlsx" : format}`;
    a.click();
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Sales invoices by customer, product, or month</p>
          </div>
        </div>
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <Card>
              ...
              <p className="text-xl font-bold text-gray-900">{data.totals.invoice_count}</p>
            </Card>
            <Card>
              <p className="text-xs text-gray-500">Subtotal</p>
              <p className="text-xl font-bold text-gray-900">{formatCurrency(data.totals.subtotal)}</p>
            </Card>
            <Card>
              <p className="text-xs text-gray-500">Tax</p>
              <p className="text-xl font-bold text-gray-900">{formatCurrency(data.totals.tax_amount)}</p>
            </Card>
            <Card>
              <p className="text-xs text-gray-500">Total</p>
              <p className="text-xl font-bold text-gray-900">{formatCurrency(data.totals.total)}</p>
            </Card>
          </div>
                ...
                <tbody>
                  {data.entries.map((entry) => (
                    <tr key={entry.id} className="border-b border-gray-50 hover:bg-gray-50">
                      ...
                      <td className="px-4 py-2 text-gray-900">{entry.contact?.name || "Walk-in"}</td>
                  ...
                  <tr className="bg-gray-50 font-semibold border-t-2 border-gray-300">
                    <td colSpan={4} className="px-4 py-3">Total ({data.totals.invoice_count} invoices)</td>
