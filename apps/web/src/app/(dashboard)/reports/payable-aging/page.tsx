...
import { usePayableAging, useExportReport } from "@/hooks/use-reports";
...
export default function PayableAgingPage() {
  ...
  const { data, isLoading } = usePayableAging({ asOfDate });
  const exportMutation = useExportReport();

  const handleExport = async (format: "pdf" | "excel" | "csv") => {
    const blob = await exportMutation.mutateAsync({
      reportType: "payable-aging",
      format,
      filters: { asOfDate },
    });
    ...
    a.href = url;
    a.download = `payable-aging-${today}.${format === "excel" ? "xlsx" : format}`;
    a.click();
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Payable Aging</h1>
          ...
          {chartData.length > 0 && (
            <Card>
              <h3 className="text-sm font-semibold text-gray-900 mb-4">
                Aging Distribution (Top 10 Vendors)
              </h3>
                ...
                <tbody>
                  {data.buckets.map((b) => (
                    <tr key={b.contact_id} className="border-b border-gray-50 hover:bg-gray-50">
                      ...
                      <td className="text-right px-4 py-2">{b.current > 0 ? formatCurrency(b.current) : "--"}</td>
                      <td className="text-right px-4 py-2">{b.days_1_30 > 0 ? formatCurrency(b.days_1_30) : "--"}</td>
                      <td className="text-right px-4 py-2">{b.days_31_60 > 0 ? formatCurrency(b.days_31_60) : "--"}</td>
                      <td className="text-right px-4 py-2">{b.days_61_90 > 0 ? formatCurrency(b.days_61_90) : "--"}</td>
                      <td className="text-right px-4 py-2 text-red-600 font-medium">{b.days_90_plus > 0 ? formatCurrency(b.days_90_plus) : "--"}</td>
                      <td className="text-right px-4 py-2 font-semibold">{formatCurrency(b.total)}</td>
                    </tr>
                  ))}
                  <tr className="bg-gray-50 font-semibold border-t-2 border-gray-300">
                    <td className="px-4 py-3">Total</td>
                    <td className="text-right px-4 py-3">{formatCurrency(data.totals.current)}</td>
                    <td className="text-right px-4 py-3">{formatCurrency(data.totals.days_1_30)}</td>
                    <td className="text-right px-4 py-3">{formatCurrency(data.totals.days_31_60)}</td>
                    <td className="text-right px-4 py-3">{formatCurrency(data.totals.days_61_90)}</td>
