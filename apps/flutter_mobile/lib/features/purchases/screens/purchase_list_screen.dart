...
class _PurchaseListScreenState extends ConsumerState<PurchaseListScreen> {
  ...
  static const _filters = ['all', 'draft', 'sent', 'overdue', 'paid'];
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final res = await api.get('/bill/purchases', queryParameters: params);
  ...
  Widget _buildContent() {
    ...
    if (_items.isEmpty) {
      return Center(
        child: Column(
          ...
          children: [
            Icon(Icons.shopping_cart_outlined, size: 64, color: AppColors.muted.withOpacity(0.4)),
            const SizedBox(height: 16),
            Text('No purchases found', style: TextStyle(color: AppColors.muted, fontSize: 16)),
          ],
        ),
      );
    }
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, i) => _buildPurchaseCard(_items[i]),
      ),
    );
  }

  Widget _buildPurchaseCard(Map<String, dynamic> item) {
    final billNumber = item['billNumber'] ?? item['bill_number'] ?? item['number'] ?? '-';
    final vendorName = item['vendorName'] ?? item['vendor_name'] ?? item['vendor']?['name'] ?? '-';
    final amount = (item['total'] ?? item['amount'] ?? item['grandTotal'] ?? item['grand_total'] ?? 0).toDouble();
    final date = item['date'] ?? item['billDate'] ?? item['bill_date'] ?? item['createdAt'] ?? '';
