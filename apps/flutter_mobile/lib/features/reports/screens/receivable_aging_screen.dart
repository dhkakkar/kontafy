...
class ReceivableAgingScreen extends ConsumerStatefulWidget {
  const ReceivableAgingScreen({super.key});

  @override
  ConsumerState<ReceivableAgingScreen> createState() =>
      _ReceivableAgingScreenState();
}

class _ReceivableAgingScreenState
    extends ConsumerState<ReceivableAgingScreen> {
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      final res = await api.get(
        '${Endpoints.reports}/receivable-aging',
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Receivable Aging'),
                    ...
                    : _buckets.isEmpty
                        ? Center(
                            child: Column(
                              ...
                              children: [
                                ...
                                const SizedBox(height: 12),
                                Text('No outstanding receivables',
                                    style: TextStyle(
                        ...
                        : RefreshIndicator(
                            ...
                            child: ListView(
                              padding: const EdgeInsets.all(16),
                              children: [
                                ..._buckets.map(_buildCustomerCard),
                                const SizedBox(height: 8),
                                _buildTotalsCard(),
                              ],
                            ),
  ...
  Widget _buildCustomerCard(Map<String, dynamic> bucket) {
