...
class _PurchaseRegisterScreenState
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      final res = await api.get(
        '${Endpoints.reports}/purchase-register',
        queryParameters: {
      ...
      final List<dynamic> rawEntries =
          data['entries'] ?? data['bills'] ?? data['purchases'] ?? data['items'] ?? [];
      final entries =
  ...
  Widget _buildSummaryCards() {
    return Row(
      children: [
        _summaryCard('Bills', '${_summary['count'] ?? 0}', AppColors.navy),
        const SizedBox(width: 8),
        _summaryCard('Subtotal', abbreviateINR(_summary['subtotal'] ?? 0),
            AppColors.blue),
        const SizedBox(width: 8),
        _summaryCard(
            'Tax', abbreviateINR(_summary['tax'] ?? 0), AppColors.amber),
        const SizedBox(width: 8),
        _summaryCard(
            'Total', abbreviateINR(_summary['total'] ?? 0), AppColors.green),
      ],
    );
  }
  ...
  Widget _buildBillCard(Map<String, dynamic> entry) {
    final billNumber =
        entry['bill_number'] ?? entry['billNumber'] ?? entry['number'] ?? '-';
    final vendorName =
        entry['vendor_name'] ?? entry['vendorName'] ?? entry['contact_name'] ?? '-';
    final date = entry['date'] ?? entry['bill_date'] ?? '';
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                _miniLabel('Total', formatINR(total), bold: true),
                _miniLabel(
                  'Balance Due',
